...
/**
 * {@code SystemLambda} is a collection of functions for testing code that uses {@code
 * java.lang.System}.
 *
 * <h2>Security Manager</h2>
 *
 * <p>The function {@link #withSecurityManager(SecurityManager, Statement) withSecurityManager} lets
 * you specify which {@code SecurityManager} is returned by {@code System.getSecurityManger()} while
 * your code under test is executed.
 *
 * <pre>
 * &#064;Test
 ...
 * </pre>
 *
 * <p>After the statement {@code withSecurityManager(...)} is executed {@code
 * System.getSecurityManager()} will return the original security manager again.
 *
 * <h2>Environment Variables</h2>
 *
 * <p>The method {@link #withEnvironmentVariable(String, String) withEnvironmentVariable} allows you
 * to set environment variables within your test code that are removed after you code under test is
 * executed.
 *
 * <pre>
 * &#064;Test
 ...
 *
 * <p>The function {@link #restoreSystemProperties(Statement) restoreSystemProperties} guarantees
 * that after executing the test code each System property has the same value like before. Therefore
 * you can modify System properties inside of the test code without having an impact on other tests.
 *
 * <pre>
 * &#064;Test
 ...
 * <h2>System.exit</h2>
 *
 * <p>Command-line applications terminate by calling {@code System.exit} with some status code. If
 * you test such an application then the JVM that runs the test would exit when the application
 * under test calls {@code System.exit}. You can avoid this with the method {@link
 * #catchSystemExit(Statement) catchSystemExit} which also provides the status code of the {@code
 * System.exit} call.
 *
 * <pre>
 * &#064;Test
 ...
 * </pre>
 *
 * The method `{@code catchSystemExit} throws an {@code AssertionError} if the code under test does
 * not call {@code System.exit}. Therefore your test fails with a failure message "System.exit has
 * not been called."
 *
 * <h2>System.out and System.err</h2>
 *
 * <p>Command-line applications usually write to the console. If you write such applications you
 * need to test the output of these applications. The methods {@link #tapSystemErr(Statement)
 * tapSystemErr}, {@link #tapSystemErrNormalized(Statement) tapSystemErrNormalized}, {@link
 * #tapSystemOut(Statement) tapSystemOut} and {@link #tapSystemOutNormalized(Statement)
 * tapSystemOutNormalized} allow you to tap the text that is written to {@code System.err}/{@code
 * System.out}. The methods with the suffix {@code Normalized} normalize line breaks to {@code \n}
 ...
 * }</pre>
 *
 * <p>You can assert that nothing is written to {@code System.err}/{@code System.out} by wrapping
 ...
 * </pre>
 *
 * <p>If the code under test writes text to {@code System.err}/{@code System.out} then it is
 * intermixed with the output of your build tool. Therefore you may want to avoid that the code
 * under test writes to {@code System.err}/{@code System.out}. You can achieve this with the
 * function {@link #muteSystemErr(Statement) muteSystemErr}/{@link #muteSystemOut(Statement)
 * muteSystemOut}. E.g. the following tests don't write anything to {@code System.err}/{@code
 * System.out}:
 *
 * <pre>
 * &#064;Test
 ...
 * <h2>System.in</h2>
 *
 * <p>Interactive command-line applications read from {@code System.in}. If you write such
 * applications you need to provide input to these applications. You can specify the lines that are
 * available from `{@code System.in} with the method {@link #withTextFromSystemIn(String...)
 ...
 * </pre>
 *
 * For a complete test coverage you may also want to simulate `System.in` throwing exceptions when
 * the application reads from it. You can specify such an exception (either `RuntimeException` or
 * `IOException` after specifying the text. The exception will be thrown by the next `read` after
 * the text has been consumed.
 *
 * <pre>
 * &#064;Test
 ...
 *
 * <ul>
 *   <li>Test code that calls {@code System.exit(int)}: {@link #catchSystemExit(Statement)}
 * </ul>
 */
public class SystemLambda {
  ...
  /**
   * Executes the statement and fails (throws an {@code AssertionError}) if the statement tries to
   * write to {@code System.err}.
   *
   ...
   * </pre>
   *
   * The test fails with the failure "Tried to write 's' to System.err although this is not
   * allowed."
   *
   * @param statement an arbitrary piece of code.
   * @throws Exception any exception thrown by the statement or an {@code AssertionError} if the
   ...
   * </pre>
   *
   * The test fails with the failure "Tried to write 's' to System.out although this is not
   * allowed."
   *
   * @param statement an arbitrary piece of code.
   * @throws Exception any exception thrown by the statement or an {@code AssertionError} if the
  ...
  /**
   * Usually the output of a test to {@code System.err} does not have to be visible. It may even
   * slowdown the test. {@code muteSystemErr} can be used to suppress this output.
   *
   * <pre>
   * &#064;Test
  ...
  /**
   * Usually the output of a test to {@code System.out} does not have to be visible. It may even
   * slowdown the test. {@code muteSystemOut} can be used to suppress this output.
   *
   * <pre>
   * &#064;Test
  ...
  /**
   * {@code tapSystemErr} returns a String with the text that is written to {@code System.err} by
  ...
  /**
   * {@code tapSystemOut} returns a String with the text that is written to {@code System.out} by
   * the provided piece of code.
   *
   * <pre>
   * &#064;Test
  ...
  /**
   * {@code tapSystemErrNormalized} returns a String with the text that is written to {@code
   * System.err} by the provided piece of code. New line characters are replaced with a single
   * {@code \n}.
   *
   * <pre>
   * &#064;Test
  ...
  /**
   * {@code tapSystemOutNormalized} returns a String with the text that is written to {@code
   * System.out} by the provided piece of code. New line characters are replaced with a single
   * {@code \n}.
   *
   * <pre>
   * &#064;Test
  ...
  /**
   * Executes the statement and restores the system properties after the statement has been
   * executed. This allows you to set or clear system properties within the statement without
  ...
  /**
   * Executes a statement with the specified environment variables. All changes to environment
   * variables are reverted after the statement has been executed.
   *
   * <pre>
   * &#064;Test
   ...
   * </pre>
   *
   * <p>You cannot specify the value of an an environment variable twice. An {@code
   * IllegalArgumentException} when you try.
   *
   * <p><b>Warning:</b> This method uses reflection for modifying internals of the environment
   * variables map. It fails if your {@code SecurityManager} forbids such modifications.
   *
   * @param name the name of the environment variable.
   * @param value the value of the environment variable.
   * @return an {@link WithEnvironmentVariables} instance that can be used to set more variables and
   *     run a statement with the specified environment variables.
   * @since 1.0.0
   ...
   *
   * @param securityManager the security manager that is used while the statement is executed.
   * @param statement an arbitrary piece of code.
   * @throws Exception any exception thrown by the statement.
   * @since 1.0.0
   */
  public static void withSecurityManager(SecurityManager securityManager, Statement statement)
      throws Exception {
  ...
  /**
   * Executes the statement and lets {@code System.in} provide the specified text during the
   * execution. In addition several Exceptions can be specified that are thrown when {@code
   * System.in#read} is called.
   *
   * <pre>
   *   public void MyTest {
   *
   ...
   *
   * <p>You can also simulate a {@code System.in} that throws an {@code IOException} or {@code
   * RuntimeException}. Use
   *
   * <pre>
   *   public void MyTest {
   *
   ...
   * </pre>
   *
   * <p>If you provide text as parameters of {@code withTextFromSystemIn(...)} in addition then the
   ...
   * @return an {@link SystemInStub} instance that is used to execute a statement with its {@link
   *     SystemInStub#execute(Statement) execute} method. In addition it can be used to specify an
   *     exception that is thrown after the text is read.
   * @since 1.0.0
  ...
  public static SystemInStub withTextFromSystemIn(String... lines) {
    String text =
        stream(lines).map(line -> line + getProperty("line.separator")).collect(joining());
    return new SystemInStub(text);
  }

  private static void executeWithSystemErrReplacement(
      OutputStream replacementForErr, Statement statement) throws Exception {
  ...
  private static void executeWithSystemOutReplacement(
      OutputStream replacementForOut, Statement statement) throws Exception {
  ...
  /**
   * A collection of values for environment variables. New values can be added by {@link
   * #and(String, String)}. The {@code EnvironmentVariables} object is then used to execute an
   * arbitrary piece of code with these environment variables being present.
   */
  public static final class WithEnvironmentVariables {
    ...
    /**
     * Creates a new {@code WithEnvironmentVariables} object that additionally stores the value for
     * an additional environment variable.
     *
     * <p>You cannot specify the value of an environment variable twice. An {@code
     * IllegalArgumentException} when you try.
     *
     * @param name the name of the environment variable.
     * @param value the value of the environment variable.
     * @return a new {@code WithEnvironmentVariables} object.
     * @throws IllegalArgumentException when a value for the environment variable {@code name} is
     *     already specified.
    ...
    /**
     * Executes a statement with environment variable values according to what was set before. All
     * changes to environment variables are reverted after the statement has been executed.
     *
     * <pre>
     * &#064;Test
     * public void execute_code_with_environment_variables(
    ...
    private void set(Map<String, String> variables, String name, String value) {
      if (value == null) variables.remove(name);
    ...
    private static Map<String, String> getEditableMapOfVariables() {
      ...
      try {
        ...
        throw new RuntimeException(
            "System Rules cannot access the field" + " 'm' of the map System.getenv().", e);
  ...
  public static class SystemInStub {
    ...
    private static class ReplacementInputStream extends InputStream {
      ...
      ReplacementInputStream(
          String text, IOException ioException, RuntimeException runtimeException) {
      ...
      private void handleEmptyReader() throws IOException {
        ...
        else if (runtimeException != null) throw runtimeException;
      }
      ...
      public int read(byte[] buffer, int offset, int len) throws IOException {
        ...
        else return readNextLine(buffer, offset, len);
      }
      ...
      private boolean contains(byte[] array, byte[] pattern, int indexStart) {
        for (int i = 0; i < pattern.length; ++i)
          if (array[indexStart + i] != pattern[i]) return false;
        return true;
      }
    }
  }
  ...
  /**
   * Executes the statement and returns the status code that is provided to {@code System.exit(int)}
   * within the statement. Additionally it avoids that the JVM is shut down because of a call to
   ...
   * @throws Exception any exception thrown by the statement.
   * @throws AssertionError if the statement does not call {@code System.exit(int)}.
   * @since 1.0.0
   */
  public static int catchSystemExit(Statement statement) throws Exception {
    NoExitSecurityManager noExitSecurityManager = new NoExitSecurityManager(getSecurityManager());
    try {
  ...
  private static int checkSystemExit(NoExitSecurityManager securityManager) {
    if (securityManager.isCheckExitCalled()) return securityManager.getStatusOfFirstCheckExitCall();
    else throw new AssertionError("System.exit has not been called.");
  }
  ...
  /**
   * A {@code NoExitSecurityManager} throws a {@link CheckExitCalled} exception whenever {@link
   * #checkExit(int)} is called. All other method calls are delegated to the original security
   * manager.
   */
